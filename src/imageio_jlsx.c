...
int JLSxEncode (uint8_t *pbuf, int *img, int *imgrcon, int ysz, int xsz, int BPP, int NEAR); // return: positive: JPEG-LS stream length , -1:failed
int JLSxDecode (uint8_t *pbuf, int *img, int *pysz, int *pxsz, int *pBPP, int *pNEAR);       // return:  -1:failed  0:success
int getJLSXHeader (uint8_t **ppbuf, int *pysz, int *pxsz, int *pBPP, int *pNEAR);            // return:  -1:failed  0:success


// return:   0 : success    1 : failed
int writeJLSXImageFile (const char *p_filename, const uint8_t *p_buf, int is_rgb, uint32_t height, uint32_t width, int near) {
    int jlsx_size;
    int *p_img_orig  = (int*)malloc(sizeof(int)*width*height*2 + 2*(width+1)*(height+1)+1048576);
    int *p_img_recon = (int*)    (p_img_orig +  width*height);
    uint8_t *p_jlsx  = (uint8_t*)(p_img_orig +  width*height*2);
    ...
    if (is_rgb) {
        ...
        printf("   warning: this JLSXencoder currently only support gray 8-bit image instead of RGB image. Only compress the green channel of this image.\n");
    ...
    jlsx_size = JLSxEncode(p_jlsx, p_img_orig, p_img_recon, (int)height, (int)width, 8, near);
    
    if (jlsx_size<=0 || height<=0 || width<=0) {
        free(p_img_orig);
        return 1;
    }
    
    {
        ...
        if (fp) {
            failed = ((size_t)jlsx_size != fwrite(p_jlsx, sizeof(uint8_t), (size_t)jlsx_size, fp));
            fclose(fp);
        }
        free(p_img_orig);
        return failed;
...
uint8_t* loadJLSXImageFile (const char *p_filename, int *p_is_rgb, uint32_t *p_height, uint32_t *p_width) {
    int height, width, bpp, near;
    ...
    {
        uint8_t jlsx_header [0x100];
        uint8_t *p_jlsx_header = jlsx_header;
        fseek(fp, 0, SEEK_SET);
        fread(jlsx_header, 1, sizeof(jlsx_header), fp);
        if ( getJLSXHeader(&p_jlsx_header, &height, &width, &bpp, &near) ) {
            fclose(fp);
            return NULL;
        }
        if ( (p_jlsx_header-jlsx_header) > jlsx_size ) {
            fclose(fp);
            return NULL;
        }
    ...
    {
        int     *p_img  = (int*)malloc(sizeof(int)*width*height + jlsx_size+32);
        uint8_t *p_img_u8= (uint8_t*)p_img;
        uint8_t *p_jlsx = (uint8_t*)(p_img +       width*height);
        ...
        fseek(fp, 0, SEEK_SET);
        if (jlsx_size != fread(p_jlsx, 1, jlsx_size, fp)) {
            fclose(fp);
            free(p_img);
            return NULL;
        }
        ...
        if ( JLSxDecode(p_jlsx, p_img, &height, &width, &bpp, &near) ) {
            free(p_img);
            return NULL;
        }
...
// Warning: This JPEG-LS extension compression algorithm is written according to the ITU-T T.870 standard, but the file header does not follow the standard.
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////



const char *title = "JLS-extension-v1\n";
...
static void setParameters (int BPP, int NEAR, int *pMAXVAL, int *pRANGE, int *pT1, int *pT2, int *pT3, int *pT4, int *pINIT_A) {
    int tmp;
    *pMAXVAL = (1 << BPP) - 1;
...
static void sampleNeighbourPixels (int *img, int xsz, int i, int j, int *a, int *b, int *c, int *d, int *e) {
...
static int gradientQuantize (int val, int T0, int T1, int T2, int T3) {
    int absval = ABS(val);
    int sign = (val < 0);
    if      (absval < T0)
        return 0;
...
static int getQ (int NEAR, int T1, int T2, int T3, int T4, int a, int b, int c, int d, int e, int *psign, int *pnearq) {
...
static int getAct (int nflat, int Nq, int Aq) {
    int act, k;
    if (nflat) {
...
static void initContext (int INIT_A, int *N, int *A, int *B, int *C) {
    int i;
...
static void updateContext (int *pNq, int *pAq, int *pBq, int *pCq, int errval) {
    int Nq=*pNq;
    int Aq=*pAq;
    int Bq=*pBq;
    int Cq=*pCq;
...
static void initBinCtxSet (BIN_CTX_t binCtxSet [][8][32]) {
    int i, j, k;
    for (i=0; i<12+256; i++)
        for (j=0; j<8; j++)
            for (k=0; k<32; k++) {
...
static ARI_CODER_t newAriEncoder (uint8_t *pbuf) {
...
static ARI_CODER_t newAriDecoder (uint8_t *pbuf) {
    ARI_CODER_t co = {0, 0xff*0xff, 0, 0, pbuf};
    co.pbuf += 2;
...
static int AriGetAvd (BIN_CTX_t *ctx, int Areg) {                        // Search of suitable Av
    static const int Av [31] = { 0x7ab6, 0x7068, 0x6678, 0x5ce2, 0x53a6, 0x4ac0, 0x4230, 0x39f4, 0x33fc, 0x301a, 0x2c4c, 0x2892, 0x24ea, 0x2156, 0x1dd6, 0x1a66, 0x170a, 0x13c0, 0x1086, 0x0d60, 0x0b0e, 0x0986, 0x0804, 0x0686, 0x050a, 0x0394, 0x027e, 0x01c6, 0x013e, 0x0100, 0x0000};
    static const int Th [30] = { 0x7800, 0x7000, 0x6800, 0x6000, 0x5800, 0x5000, 0x4800, 0x4000, 0x3c00, 0x3800, 0x3400, 0x3000, 0x2c00, 0x2800, 0x2400, 0x2000, 0x1c00, 0x1800, 0x1400, 0x1000, 0x0e00, 0x0c00, 0x0a00, 0x0800, 0x0600, 0x0400, 0x0300, 0x0200, 0x0180, 0x0001 };
...
static void AriUpdateCounter (BIN_CTX_t *ctx, int bin) {
...
static void AriEncode (ARI_CODER_t *co, BIN_CTX_t *ctx, int bin) {
    ...
    if ( co->Areg < 0x100 ) {                                       // Renormalization of Areg and Creg and output data bit stream ---------------------------------------------
        ...
        *(co->pbuf++) = co->Buf1;                                   // AppendByteToStream
        co->Buf1 = co->Buf0;
        co->Buf0 = (uint8_t)( ((co->Creg >> 8) + co->Creg + 1) >> 8 );
        co->Creg += co->Buf0;
...
static void AriEncodeFinish (ARI_CODER_t *co) {
    int i;
    for (i=0; i<4; i++) {
        if( co->Creg >= 0xff*0xff ) {
            co->Creg -= 0xff*0xff;
...
static int AriDecode (ARI_CODER_t *co, BIN_CTX_t *ctx) {
    const int Avd = AriGetAvd(ctx, co->Areg);
    int bin;
...
static void treePosUpdate (int *pact, int *pver) {
    int  act = *pact;
    int  ver = *pver;
...
static void encodeMerrval (ARI_CODER_t *co, BIN_CTX_t binCtxSet [][8][32], int merrval, int act) {
...
static int decodeMerrval (ARI_CODER_t *co, BIN_CTX_t binCtxSet [][8][32], int act) {
...
static void putHeader (uint8_t **ppbuf, int ysz, int xsz, int BPP, int NEAR) {
    int i;
    for (i=0; title[i]!=0; i++)              // put title
        *((*ppbuf)++) = (uint8_t)title[i];
    *((*ppbuf)++) = (uint8_t)BPP;            // put BPP
    *((*ppbuf)++) = (uint8_t)NEAR;           // put NEAR
    *((*ppbuf)++) = (uint8_t)(xsz >>  8);    // put image width 
    *((*ppbuf)++) = (uint8_t)(xsz >>  0);
    *((*ppbuf)++) = (uint8_t)(ysz >>  8);    // put image height 
    *((*ppbuf)++) = (uint8_t)(ysz >>  0);
}
...
int getJLSXHeader (uint8_t **ppbuf, int *pysz, int *pxsz, int *pBPP, int *pNEAR) {
    int i;
    for (i=0; title[i]; i++)                    // check title 
        if ( *((*ppbuf)++) != (uint8_t)title[i] )
            return -1;
    *pBPP  =   *((*ppbuf)++);                   // get BPP 
...
int JLSxEncode (uint8_t *pbuf, int *img, int *imgrcon, int ysz, int xsz, int BPP, int NEAR) {
    int  MAXVAL, RANGE, T1, T2, T3, T4, INIT_A;
    
    int  i, j;
    
    uint8_t *buf_base = pbuf;
...
// return:  -1:failed  0:success
int JLSxDecode (uint8_t *pbuf, int *img, int *pysz, int *pxsz, int *pBPP, int *pNEAR) {
    ...
    if ( getJLSXHeader(&pbuf, &ysz, &xsz, &BPP, &NEAR) )
        return -1;
